...
export const addToWatchlist: RequestHandler = async (req, res, next): Promise<void> => {
  // Now also receive "industry" from the client
  const { stock_symbol, quantity, industry } = req.body;
  const userId = (req as any).user.id;
  if (!stock_symbol) {
...
export const removeFromWatchlist: RequestHandler = async (req, res, next): Promise<void> => {
  const { id } = req.params; // Use the unique watchlist record id
  const userId = (req as any).user.id;
  if (!id) {
